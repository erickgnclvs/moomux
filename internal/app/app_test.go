...
func TestCreateSessionModelAppendsFlag(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.agent+"/"+tc.model, func(t *testing.T) {
			...
			if _, _, err := a.CreateSession("demo", "feat", tc.agent, "", "", true, false, "", tc.model, ""); err != nil {
				t.Fatal(err)
			}
			found := false
			for _, c := range tm.calls {
...
func TestCreateSessionThinkingAppendsCodexFlag(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.agent+"/"+tc.thinking, func(t *testing.T) {
			...
			if _, _, err := a.CreateSession("demo", "feat", tc.agent, "", "", true, false, "", "", tc.thinking); err != nil {
				t.Fatal(err)
			}
			found := false
			for _, c := range tm.calls {
...
func TestCreateSessionBaseBranchOverride(t *testing.T) {
	...
	s, _, err := a.CreateSession("demo", "feat", "", "", "", true, false, "develop", "", "")
	if err != nil {
		t.Fatal(err)
	}
	if s.Branch != "feat" {
