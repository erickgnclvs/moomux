...
type Session struct {
	...
	Folder       string    `json:"folder,omitempty"`      // name of a collapsible group (config.Config.Folders) this session is filed under; the namespace is global, not scoped to this session's project, so the same name groups sessions across projects; "" = top-level
}
