...
const (
	agentCommandPath       = "~/.local/share/moomux/bin/agent-command"
	...
	killSkillInstructions = `Run ` + "`" + agentCommandPath + ` park` + "`" + ` in a shell with ` + "`sandbox_permissions`" + ` set to ` + "`require_escalated`" + `. If shell commands must use RTK, run ` + "`rtk " + agentCommandPath + ` park` + "`" + `. Do not substitute an executable named ` + "`moomux`" + `: RTK applies a command-specific proxy to that name instead of reliably executing the installed binary. The tmux socket is outside Codex's sandbox, so a sandboxed run cannot park the session. Request approval through the shell tool without asking a separate confirmation question, and do not retry inside the sandbox if approval is denied. The command stops the tmux session and closes its terminal tab, but keeps the worktree and branch so the session can be reopened later (the same behavior as moomux's ` + "`x`" + ` key). Report success only if the command succeeds; otherwise say the session was not parked and include the error.
`

	tagSkillInstructions = `Use ` + "`" + agentCommandPath + "`" + ` for every command below, with ` + "`sandbox_permissions`" + ` set to ` + "`require_escalated`" + `. If shell commands must use RTK, prefix that expanded path with ` + "`rtk`" + `. Do not substitute an executable named ` + "`moomux`" + ` because RTK proxies that name.

Run ` + "`" + agentCommandPath + ` tag` + "`" + ` with no flags first to see what is already tracked on this session.

Find the open pull request for the current branch and run:

    ` + agentCommandPath + ` tag -pr <that PR URL>
...
Leave out ` + "`-ticket`" + ` because moomux keeps the existing ticket when that flag is omitted. If no ticket is tracked yet, look for a ticket link in the pull request title or body, branch name, and recent commit messages. Recognize Asana, Jira, and Linear ticket references. If you find one, pass it too:
    ...
    ` + agentCommandPath + ` tag -pr <PR URL> -ticket <ticket URL>

If there is no open pull request, say so instead of guessing. Do not guess a ticket link either.
`

	spawnSkillInstructions = `Use ` + "`" + agentCommandPath + "`" + ` for every command below, with ` + "`sandbox_permissions`" + ` set to ` + "`require_escalated`" + `. If shell commands must use RTK, prefix that expanded path with ` + "`rtk`" + `. Do not substitute an executable named ` + "`moomux`" + ` because RTK proxies that name.

Use the task description supplied with this invocation as literal text. Do not resolve issue-like tokens such as ` + "`#123`" + ` unless the user explicitly asks you to.

Run ` + "`" + agentCommandPath + ` spawn -list` + "`" + ` and match the current repository against a listed project name. If the task explicitly names a different project, use that instead. Ask the user if no project can be matched safely.
...
Write a clear, self-contained task prompt because the spawned agent starts with no context beyond that prompt. Derive a short kebab-case session name, then run:
    ...
    ` + agentCommandPath + ` spawn -project <project> -name <name> -prompt "<task prompt>"

This is fire-and-forget. Do not wait for or check the spawned session's progress.
`

	reseedSkillInstructions = `Run ` + "`" + agentCommandPath + ` reseed` + "`" + ` in a shell with ` + "`sandbox_permissions`" + ` set to ` + "`require_escalated`" + `, without asking a separate confirmation question first. If shell commands must use RTK, run ` + "`rtk " + agentCommandPath + ` reseed` + "`" + `. Do not substitute an executable named ` + "`moomux`" + ` because RTK proxies that name. This re-runs the current session's worktree-create userscripts with ` + "`MOOMUX_FORCE=1`" + `. Then report what it printed.
`
)
